...
export default function UserSidebar({ isOpen, onClose }) {
  ...
  const recruitNewAgents = settings?.recruitNewAgents !== false;
  ...
  const handleSwitchToAgent = async () => {
    try {
      const res = await upgradeService.switchRole('agent');
      if (res.success) {
        toast.success('Switched to Agent Dashboard');
        if (res.user) {
          setUser(res.user);
        } else {
          setUser(prev => ({ ...prev, role: 'agent' }));
        }
        ...
        <div className="flex flex-col h-full bg-gradient-to-b from-white via-white to-slate-50/50">
          ...
          <div className="p-6 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <h2 className="text-lg font-bold text-slate-900 tracking-tight leading-none">User Account</h2>
                <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest mt-1">Personal Dashboard</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto scrollbar-hide">
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <Icon size={18} className={`${active ? 'text-blue-400' : 'text-slate-600 group-hover:text-slate-700'} transition-colors`} />
            ...
            {/* Switch/Become Agent Section */}
            {(user?.agentFeeStatus === 'paid' || user?.agentFeeStatus === 'protocol' || user?.role === 'agent') ? (
              <button
              ...
              >
                <ArrowRightLeft size={18} className="text-slate-400 group-hover:text-blue-600" />
                <span className="font-bold text-sm tracking-tight">Switch to Agent</span>
              </button>
            ) : recruitNewAgents ? (
              <Link
                to="/become-agent"
                onClick={onClose}
                className={`group flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all duration-300 mt-4 border border-dashed border-blue-200 hover:bg-blue-50/50 ${isActive('/become-agent') ? 'bg-blue-50 border-blue-300 text-blue-700' : 'text-slate-600'}`}
              >
                <div className="flex items-center gap-3">
                  <ShieldCheck size={18} className="text-blue-600" />
                  <span className="font-bold text-sm tracking-tight">Become an Agent</span>
                </div>
                <ArrowRightLeft size={14} className="opacity-40" />
              </Link>
            ) : (
              <div
                className="group flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all duration-300 mt-4 border border-dashed border-slate-200 bg-slate-50/50 text-slate-400 cursor-not-allowed"
                title="Agent recruitment is currently closed"
              >
                <div className="flex items-center gap-3">
          ...
          <div className="p-4 border-t border-slate-100 bg-white/50 backdrop-blur-md text-center">
            <div className="flex items-center gap-3 p-2.5 rounded-2xl bg-slate-100/50 mb-3 text-left">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs">
                {user?.name?.slice(0, 2).toUpperCase() || 'US'}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-slate-900 truncate">{user?.name || 'User'}</p>
                <p className="text-[9px] font-medium text-slate-600 truncate uppercase tracking-tighter">Verified Member</p>
              </div>
            </div>
